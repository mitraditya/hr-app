...
export const changelog: ChangelogRelease[] = [
  {
    ...
    entries: [
      { type: 'improvement', description: 'The send history now records why a message went out in its plain version. When the model is busy or returns something unusable the email still sends — that is the point of keeping a hand-written version — but until now the record looked identical to a template with AI switched off, so a run that quietly stopped using AI was invisible unless you read the server logs. The reason is now stored alongside the send and can be queried from Ask AI.' },
      { type: 'fix', description: 'Ask AI reported a busy model as a 502 Bad Gateway. The status now matches what actually happened — 429, too many requests — so the browser console says the model is rate-limited instead of looking like a broken deployment. The distinction is now carried as a value through the code rather than inferred from the wording of an error message.' },
      { type: 'feature', description: 'The Ask AI reporting box now has its own provider and model picker, drawn from the same live OpenRouter list. It previously borrowed whichever template happened to be selected in the Templates tab, and used a fixed default when none was — so a question could fail for a reason that had nothing to do with the question, and nothing on screen explained why. Your choice here is remembered between visits, since free models are rate-limited often enough that re-picking every time would grate.' },
      { type: 'fix', description: 'Explained the 502 error that Ask AI returned when its model was busy. A rate-limited model made the request fail with a bare gateway error and the provider’s raw text, which reads like a broken deployment. Free models queue behind every other free user, so being temporarily busy is ordinary; the message now says so and points at the model picker.' },
      { type: 'feature', description: 'The model field in the AI Email section is now a list you pick from rather than a slug you have to type from memory. It is read live from OpenRouter each time the page loads, so it always reflects what is actually available today — the free tier changes constantly, and a list written into the app would be wrong within weeks. Free models are shown by default with a switch to include paid ones, each entry shows how much context it holds, and any model that cannot be asked for JSON directly is marked as such, because those fall back to the plain version more often. A model you have already saved that has since been withdrawn still appears, labelled, instead of quietly changing to something else. You can still type a slug by hand, and if OpenRouter cannot be reached the field simply reverts to a text box rather than blocking the page.' },
      { type: 'improvement', description: 'Preview and Send-test now use the provider and model currently on screen instead of the ones last saved. Previously you had to save a template before you could find out whether its model worked, which meant committing a choice to test it. This matters most when a free model is rate-limited: those queue behind every other free user on the provider, and being temporarily busy is normal rather than a sign of a fault. You can now work down the list until one answers, then save. Nothing is stored until you press Save, and the rate-limit message says so explicitly rather than reading like a configuration error.' },
      { type: 'fix', description: 'Fixed the AI email writer reporting that its API key was not configured when the key had in fact been set. The key had been added to the website hosting environment, but the code that reads it runs on the database platform instead, where the two do not share settings — so the value was set somewhere it could never be seen from. It now lives where it is actually read.' },
      ...
      { type: 'fix', description: 'Fixed buttons arriving with no visible text. The label was always there \u2014 it was being painted in the same colour as the button behind it, so it vanished. The styling that turns a link blue was running over the finished button and overriding its white text. Buttons are now set aside while the rest of the message is styled and put back afterwards, so nothing can paint over them. Added tests that catch this exact fault, checked by reintroducing the bug and confirming they fail.' },
      { type: 'improvement', description: 'Automated emails now go out in a proper branded layout instead of bare unstyled text. There is a header carrying the OpenHRApp name, readable typography and spacing, a coloured action button, and a footer with the unsubscribe link. It is built the way email has to be built rather than the way a web page is \u2014 tables, inline styles, explicit widths \u2014 so it holds together in Outlook and does not become dark text on a dark background in Gmail\u2019s dark mode. The layout lives in one place, so improving it improves every message without touching a single template.' },
      { type: 'feature', description: 'Added a composer for writing email bodies, replacing the raw HTML box. Bold, italic, headings, bulleted and numbered lists, links, undo and redo, and one-click buttons to drop in the recipient\u2019s name or their organization name. You can insert a coloured call-to-action button in six colours and set where it links to, and it appears as a button while you write rather than as raw markup. Pasting from Word or a web page brings the text and leaves the mess behind. There is still a direct HTML view, and anything outside the handful of tags email clients render reliably is stripped rather than sent and hoped for.' },
      { type: 'improvement', description: 'Rewrote the wording of the supplied emails. Buttons said \u201cOpen OpenHRApp\u201d, which names the tool rather than what happens when you click it, and repeats a brand already printed at the top of the message. They now say things like \u201cOpen your dashboard\u201d and \u201cAdd your team\u201d. Subject lines lead with the organization name, which is the part a reader recognises in a crowded inbox. A template you have already edited yourself is left untouched.' },
      { type: 'improvement', description: 'The preview now shows the finished email exactly as it will arrive, framed and styled, rather than a rough approximation. It is rendered in isolation so the email\u2019s own styling cannot bleed into the dashboard around it.' },
      { type: 'feature', description: 'Nine audiences to send to instead of four. Added: organizations that never finished setup, trials that have already lapsed, organizations that are set up and running, ones that were active and went quiet, and healthy active organizations for product news. Because the timing means something different for each \u2014 days since signing up, days remaining on a trial, days since the last check-in \u2014 the field now says which one applies to the audience you picked instead of leaving you to guess.' },
      { type: 'fix', description: 'Corrected the product name across the app and every automated email. Messages were going out from \u201cOpenHR\u201d rather than \u201cOpenHRApp\u201d, and the daily attendance report signed off \u201cAutomated daily report from OpenHR\u201d. The changelog page title also read \u201cWhat\u2019s New in OpenHR | OpenHRApp\u201d. One email function had already been corrected at some point, which is what made the rest stand out as stale.' },
      { type: 'feature', description: 'Super admins can now build their own automated email templates rather than only editing the four supplied ones, and delete templates they no longer want. A new template always starts switched off, so you can write it, preview it and send yourself a test before anything reaches a customer. Deleting a template keeps the record of what it already sent \u2014 the history outlives the template on purpose. A template has to be paused before it can be deleted.' },
      { type: 'feature', description: 'You can now ask questions about your customer base in plain English from the Email section \u2014 \u201cwhich organizations never confirmed their admin email?\u201d, \u201ctrials ending in the next 7 days\u201d \u2014 and get a table back with a short written answer. It is built for deciding who is worth emailing before you switch an automated template on. The query it wrote is always shown alongside the result, because a generated query you cannot inspect is a query you cannot check.' },
      { type: 'security', description: 'That reporting feature reads through four purpose-built views and runs with your own database permissions, so it can never return anything you could not already see, and it cannot reach login records at all. Only single read-only queries are accepted, results stop at 200 rows, and a query that runs too long is cut off. Your organization names and email addresses are written by whoever signed up, so they are treated as untrusted text throughout rather than as instructions.' },
      { type: 'improvement', description: 'Bulk email and the automated templates now live together under a single Email tab instead of two separate ones, with the send history, the do-not-email list and the reporting tool alongside them. The super admin dashboard had grown to fourteen top-level tabs; this removes one and puts everything about email in one place.' },
      { type: 'feature', description: 'Added an AI Email section to the super admin dashboard. Four onboarding and reminder emails — confirm your email, add your first employee, how your team checks in, and trial ending — each with an instruction you write in plain English telling the model what tone to take and what to mention. Preview any of them against real organization data, or send yourself a test. Test sends go only to your own address and can never reach a customer.' },
      { type: 'feature', description: 'The email writer supports OpenRouter, DeepSeek, OpenAI and Anthropic, chosen per template, so you can run routine onboarding copy on a free model and reserve a paid one for anything that matters. Every template also keeps a plain, hand-written version, which is what sends whenever the model is unavailable or returns something unusable — an email that cannot go out without the AI would stop working the day the provider has an outage.' },
      { type: 'security', description: 'Every automated email carries a one-click unsubscribe, and unsubscribes are honoured permanently. All four templates arrive switched off and only send once you turn them on. Nobody can receive the same message twice, each template has a daily limit you set, and there is a do-not-email list you can add to by hand. This matters because most of the unconfirmed addresses on the platform came from the unprotected signup form, and mailing them in bulk would push genuine OpenHRApp mail into spam folders.' },
      { type: 'feature', description: 'Added an organization Review tab to the super admin dashboard. It scores every organization against the signals that separate a real customer from the residue of an unprotected signup form — never used, one account only, email never confirmed, throwaway email domain, placeholder name, trial long expired, dormant — and explains each warning in plain terms rather than showing a bare score. Organizations under a week old are deliberately not scored, because a genuine customer looks identical to a bot on day one.' },
      { type: 'feature', description: 'Super admins can now permanently delete organizations from that Review tab, individually or in bulk. Deletion removes the organization and everything belonging to it, including login accounts, and cannot be undone — so it requires ticking each organization, typing a confirmation phrase, and passing a separate warning that lists any selected organization holding real employee data. The demo organization cannot be selected at all. Deletions run one at a time so a partial failure is clear, and every one is recorded in the audit trail before it happens.' },
      ...
      { type: 'security', description: 'Two email endpoints checked that the caller was signed in but not who they were. One would email any leave request\'s full details — employee name, dates, reason and remarks — to anyone who asked for it by ID, regardless of which organization it belonged to. The other accepted a subject line and message body from the caller and sent them, from the OpenHRApp address, to any organization\'s administrators. Both now verify that the caller belongs to the organization concerned.' },
    ],
  },
  {
    date: '2026-08-22',
  ...
  },
  {
    ...
    entries: [
      { type: 'fix', description: 'The product name appeared four different ways. The codebase used OpenHRApp 213 times, but 26 user-visible strings still said plain OpenHRApp — including every one of the seven feature page titles, the blog and guide titles, and the RSS feed title in middleware.ts. Those titles are what search engines display, so the brand was reading as OpenHRApp in listings while the site itself said OpenHRApp. All 26 are now OpenHRApp.' },
      ...
      { type: 'improvement', description: 'The organisation-byline check in the prerender middleware was rewritten as a single pattern that still matches the legacy spellings. Rows written before this change carry bylines of OpenHRApp and OpenHRApp Team, and treating those as a Person rather than an Organization emits structured data that Google flags as invalid. Historical changelog entries and the test asserting the legacy match are deliberately left alone.' },
    ],
  },
  {
    date: '2026-08-21',
  ...
  },
  {
    ...
    entries: [
      ...
      { type: 'improvement', description: 'Updated the Supabase "Confirm Signup" email template with professional branding: subject line changed to "Welcome to OpenHRApp — Verify Your Email", gradient purple header, contextual body copy, and clean footer. Template HTML documented in Others/VERIFICATION_FIX_2026-07-30.md.' },
  ...
  },
  {
    ...
    entries: [
      ...
      { type: 'improvement', description: 'Added complete-guide-openhrapp.html blog post — full rewrite of the OpenHRApp complete guide with PocketBase→Supabase content updates, clean HTML format, 3 inline image placeholders, FAQ section, and updated git URLs.' },
    ],
  },
  {
    date: '2026-07-18',
  ...
  },
  {
    ...
    entries: [
      ...
      { type: 'feature', description: 'Created 16 seed blog post markdown files in seed-data/blog-posts/ covering HR management, OpenHRApp feature guides, industry insights, and company content — each 800-1,500+ words.' },
